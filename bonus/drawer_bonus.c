...
/*   Updated: 2021/11/27 21:26:17 by dchonvil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_on_screen(t_windmng *wind, int c)
{
	...
	if (c == 1)
	{
		...
		mlx_string_put(wind->mlx_id, wind->wind_id, 5,
			(wind->map_y + 10), blk, mn);
		mlx_string_put(wind->mlx_id, wind->wind_id, 5,
			(wind->map_y + 10), wht, n);
		mlx_string_put(wind->mlx_id, wind->wind_id, 25,
			(wind->map_y + 10), wht, "movement(s)");
		free(n);
		free(mn);
	}
	else
